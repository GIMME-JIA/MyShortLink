package org.jia.mylink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import org.jia.mylink.gateway.config.Config;
import org.jia.mylink.gateway.dto.GatewayErrorResult;
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                Object userInfo;
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get("short-link:user:login:" + username, token)) != null) {
